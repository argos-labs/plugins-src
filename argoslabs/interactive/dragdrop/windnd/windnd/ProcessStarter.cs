...
namespace windnd
{
    public class ProcessStarter : IDisposable
    {
        ...
        public static IntPtr GetForcedCurrentUserToken()
        {
            return IntPtr.Zero;
        }
        ...
        public static IntPtr GetCurrentUserToken()
        {
            ...
            for (int i = 0; i < dwCount; i++)
            {
                ...
                //Logger.Info("Session Info  : {0}, {1}, {2}", si.SessionID, si.State, si.pWinStationName);
            ...
            if (bRet == false)
            {
                ...
                //Logger.Info("WTSQueryUserToken fail  : {0}, {1}, {2}", dwSessionId, currentToken, erroCode);
                return IntPtr.Zero;
            }
            ...
            if (bRet == false)
            {
                //Logger.Info("WTSQueryUserToken DuplicateTokenEx  : {0}, {1}", dwSessionId, currentToken);
                return IntPtr.Zero;
            }
        ...
        public int Run(String workingDirectory = "")
        {
            IntPtr primaryToken = GetCurrentUserToken();
            if (primaryToken == IntPtr.Zero)
            {
                //Logger.Info("primaryToken IS NULL : !!!");
                return 0;
            }           
            ...
            CreateProcessAsUser(primaryToken, null, command, 
                ref Security1, ref Security2, false, 
                CREATE_NO_WINDOW | NORMAL_PRIORITY_CLASS | CREATE_UNICODE_ENVIRONMENT, 
                lpEnvironment, 
                String.IsNullOrWhiteSpace(workingDirectory) ? null : workingDirectory, 
                ref StartupInfo, out processInfo_);            
